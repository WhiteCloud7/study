...
```
这里要初始化SqlSessionFactory这个静态资源，故用到了静态代码块。**另外，静态代码块只初始化在对应属性的类加载时执行，且只执行一次，往后的继承重写并不会改变。**
---
# 一些方法及一些必要的基本概念
## newString():
...
- 引用类型的参数传递是引用传递，即传递的是引用的副本，即对象在内存中的地址，对副本的修改会影响原变量。如数组、collection、map等。
## 变量初始化：
