...
- 字符：char[0，$2^16$]
- 字符串：String 可以存储任意长度且不经常变化的的字符串 变化的考虑StringBuffer、StringBuilder
## 数组和集合存储：
- 数组：存储固定长度的相同类型的数据，数组的长度在创建时就确定，无法改变，**最大的长度为int的最大值**。
- 集合：存储可变长度的不同类型的数据，集合的长度可以动态改变。
  - 集合的分类：
    - 集合的分类：
      - Collection：存储一组不唯一、无序的对象。
        - List：存储一组不唯一、有序（插入顺序）的对象。
          - ArrayList：基于数组实现，动态数组，支持随机访问。
          - LinkedList：基于双向链表实现，不支持随机访问。
          - Vector：基于数组实现，线程安全。
        - Set：存储一组唯一、无序的对象。
          - HashSet：基于哈希表实现，不保证元素的顺序。
          - LinkedHashSet：基于哈希表和链表实现，保证元素的插入顺序。
          - TreeSet：基于红黑树实现，元素按照自然顺序排序。
      - Map：存储一组键值对，每个键对应一个值。
