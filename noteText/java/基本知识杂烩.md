...
由于实现了乐观读不会再获取锁失败时阻塞，所以不需要用循环或其他方式来确保读到最新的数据从而降低性能，并且通过检查戳记可以判断数据是否被修改，进一步提高了性能。  
StampedLocky也是读锁和写锁，还可以获得乐观读锁（tryOptimisticRead()）  
虽然StampedLock再保留ReentrantReadWriteLock原有读锁和写锁机制，但其 ***没有可重入锁功能也没有实现Condition接口***，所以并不能完全取代ReentrantReadWriteLock，及使用场景为 ***在ReentrantReadWriteLock的场景下且不需要可重入锁和Condition接口功能。***
#### ***加锁方法***
...
另外，实例变量：***在类内定义但不在方法内、构造函数内或代码块（即非局部变量）的非静态、非常量的变量。实例变量也是存在堆上的***。即持久化数据，但为什么没有static和常量呢，因为它们存在了方法区，他们更符合方法区的特性。  
堆是线程安全的，栈是线程不安全的。
## 常量池
