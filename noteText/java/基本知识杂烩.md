...
有以下几种常用方法：
- lock(): 获取锁，如果锁已被其他线程获取，则当前线程会被阻塞，直到锁可用。
- tryLock(): 获取锁，如果锁可用，则获取锁并立即返回true，否则返回false。也可以设置超时时间。
...
#### 四种方法使用场景
那么这里可以总结一下：***对于简单的对一个共享变量的修改，用volatile即可; 对于更复杂的共享资源用synchronized，而对于更细粒度的控制需要实现不同Lock来解决，对于不需要更改的共享资源用final即可。***
### 解决有序性：
...
- 同步代码块就是将代码块用synchronized修饰，格式为：`synchronized(指定代码块对象){代码块内容}`  
synchronized无法向Lock手动释放锁，**它会在方法或代码块执行完或者抛出异常时自动释放锁**。  
...
关于ReentrantLock怎么实现公平和非公平锁看源码或里有，这里就不展开了。我们直说怎么用它，很简单只要 ***在用构造方法创建ReentrantLock时传入true或false即可，默认不写是true即非公平锁。***
...
至于为什么适用读多写少，比如多个线程读一个数据，其中一个线程写了，那么其他线程读的就不是最新的数据了，这时 ***就需要将读操作放在循环里或其他操作增加读的次数或其他原理来确保读到最新的数据***。这样就需要读多写少。  
ReentrantReadWriteLock需要通过*ReadLock和WriteLock*来获取读锁和写锁，**读锁如上只能是共享锁，写锁如上是互斥的，其他如上有ReentrantLock可以实现的功能。**
#### StampedLock
...
可以看出CountDownLatch更适合一个或多个线程等待其他线程完成操作后再继续执行的情况。而CyclicBarrier更适合更复杂的情况，如一个或多个线程需要等待一组线程先完成，但一组线程内部之间的线程又需要 ***相互等待***，如其内部分为几个阶段，每个阶段有一个或多个线程，某个阶段要等待另一个阶段完成在执行。容易看出其实用多个CountDownLatch也能实现，但用多个CountDownLatch过于复杂且代码耦合度过高。所以对于这种 ***相对等待的情况应用CyclicBarrier***
#### Semaphore
...
当基本数据类型被初始化时，系统会自动为其分配内存空间，并将其存储在栈上。基本数据类型的内存空间是在栈上分配的，其生命周期与方法的执行相关，当方法执行完毕后，其内存空间会被自动释放。而引用数据类型被初始化时，系统会为其分配内存空间，并将其存储在堆上。引用数据类型的内存空间是在堆上分配的，其生命周期与方法的执行无关，只有当引用数据类型的所有引用都被释放后，其内存空间才会被自动释放。  
