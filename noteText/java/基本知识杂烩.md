...
- 字符：char[0，$2^16$]
## 数组和集合存储，需要结合数据结构理解：
使用场景:***数组适合固定大小、要频繁高性能访问的场景；集合适合动态大小、多样化、多对象操作的场景；字符串适合处理文本信息。***
...
- 集合：存储可变长度的不同类型的数据，集合的长度可以动态改变。
  - 集合的分类：
    - 集合的分类：
      1. Collection：存储一组不唯一、无序的对象。
        - List：存储一组不唯一、有序（插入顺序）的对象。
          - ArrayList：基于数组实现故插入和删除效率低，动态数组，支持随机访问，线程不安全。
            常用方法：add(),get(),remove(),set(),size(),contains(),isEmpty(),indexOf()(返回数据索引),clear(),equals()等等
          - LinkedList：基于双向链表实现故插入和删除效率高，不支持随机访问，线程不安全。
            常用方法：add(),addFirst()(开头加),addLast()(结尾加,get、remove、set等都一样，略),contains(),isEmpty(),indexOf()(返回数据索引),clear(),equals()等等
      ...
      2. Map：存储一组键值对，每个键对应一个值。
        - HashMap：基于哈希表实现，不保证键的顺序。
        - LinkedHashMap：基于哈希表和链表实现，保证键的插入顺序。
        - TreeMap：基于红黑树实现，键按照自然顺序排序。
        - Hashtable：基于哈希表实现，线程安全。
        - Properties：继承自Hashtable，用于存储属性键值对。