...
那么除了以上还能想到可以将字节数组转化为字节流，就是直接创造一个输入或输出处理流对象，参数是你要转的字节流即可。
以及反过来，那么前提是流里面字节数组有数据可以转换，这是有的话就用输出处理流write方法一下即可。
...
那么除了以上还能想到可以将字符串等转化为字节流，就是直接创造一个输入或输出处理流对象，参数是你要转的字节流即可。
以及反过来，那么前提是流里面字符串等有数据可以转换，这是有的话就用输出处理流write方法一下即可。`stringBuilder.insert(position, toInsert);`
